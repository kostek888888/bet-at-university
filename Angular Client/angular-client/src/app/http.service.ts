...
export class HttpService {
  ...
  getFixturesByMatchday(matchday: string) {
    return this.http.get('http://api.football-data.org/v1/competitions/' +
  ...
  getNextFixturesByTeam(teamId: string, nextDays: string = '7') {
    return this.http.get('http://api.football-data.org/v1/teams/' + teamId + '/fixtures?timeFrame=n' +
     nextDays, { headers: this.apiKeyHeader });
  }

  // to co wyzej tylko poprzednie dni
  getPastFixturesByTeam(teamId: string, pastDays: string = '7') {
    return this.http.get('http://api.football-data.org/v1/teams/' + teamId + '/fixtures?timeFrame=p' +
